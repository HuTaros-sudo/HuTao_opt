...
def test_app_startup():
    ...
    assert "Akasha較正推定Damage" in metric_labels
...
def test_simple_dust_defaults_are_editable_and_update_expected_damage():
    ...
    expected_before = next(metric.value for metric in app.metric if metric.label == "期待表示Damage")
    probability.set_value(50.0).run(timeout=15)
    expected_after = next(metric.value for metric in app.metric if metric.label == "期待表示Damage")
    assert float(expected_after) > float(expected_before)
    width = next(field for field in app.number_input if field.label.startswith("更新時の改善幅"))
    width.set_value(0.0).run(timeout=15)
    expected_zero = next(metric.value for metric in app.metric if metric.label == "期待表示Damage")
    current = next(metric.value for metric in app.metric if metric.label == "現在Akasha較正推定Damage")
...
def test_app_calculates_reshape_and_optimal_set_decision():
    ...
    assert "元に戻せることを考慮した期待表示Damage" in metric_labels
...
def test_adoption_gui_copies_original_and_uses_akasha_score() -> None:
    ...
    assert "元の場合の最適Akasha較正推定Damage" in metric_labels
    assert "再構築後の最適Akasha較正推定Damage" in metric_labels
...
def test_calibration_toggle_changes_display_only() -> None:
    app_path = Path(__file__).resolve().parents[1] / "app.py"
    app = AppTest.from_file(str(app_path)).run(timeout=15)
    toggle = next(box for box in app.checkbox if box.label == "Akasha表示補正を使用")
    calibrated_current = float(next(metric.value for metric in app.metric
                                    if metric.label == "現在装備のAkasha較正推定Damage"))
    improvement_before = next(metric.value for metric in app.metric if metric.label == "改善率")
    toggle.set_value(False).run(timeout=15)
    core_current = float(next(metric.value for metric in app.metric if metric.label == "現在装備のCore推定Damage"))
    improvement_after = next(metric.value for metric in app.metric if metric.label == "改善率")
    assert calibrated_current / core_current == pytest.approx(1.0768838049, rel=1e-5)
    assert improvement_after == improvement_before
    source = app_path.read_text(encoding="utf-8")
    assert "score_function = lambda build: score_hutao_akasha(build, config).aggregate_score" in source
    assert "optimize(scoring_inventory, score_function, constraint)" in source
